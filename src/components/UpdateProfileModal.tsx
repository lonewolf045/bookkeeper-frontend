...
export default function UpdateProfileModal({ onClose }: Props) {
  ...
  const handleUpdate = async () => {
    if (!displayName) { setError("Display name cannot be empty"); return; }
    setLoading(true);
    try {
  ...
  return (
    ...
    >
      <div role="dialog" aria-modal="true" aria-labelledby="profile-title"
        ...
        <h2 id="profile-title" className="text-2xl font-bold text-gray-800"
          style={{ fontFamily: "'Balsamiq Sans', cursive" }}>Update Profile</h2>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input ref={firstInput} type="text" value={displayName}
